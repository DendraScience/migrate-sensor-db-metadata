CREATE OR REPLACE VIEW `vw_export_datastreams_seasonal_sum` AS
SELECT 
...
`datapoints_config$0$params$query$datastream_id`+40000 AS `datapoints_config$0$params$query$datastream_id`,
`datapoints_config$0$params$query$time_adjust`,
"/legacy/datavalues-day" AS `datapoints_config$0$path`,
CONCAT("Derived from legacy datastreamid ",CAST(`datapoints_config$0$params$query$datastream_id` AS CHAR)) AS `derivation_description`,
`description`,
`enabled`,
CAST(`datapoints_config$0$params$query$datastream_id`+40000 AS CHAR(50)) AS`external_refs$0$identifier`,
...
CONCAT(TRIM(`name`)," Daily Total") AS `name`,
`organization_id`,
...
`tags$1`,
`tags$2`,
"ds_Aggregate_Cumulative" AS `tags$3`,
"ds_Interval_Day" AS `tags$4`
FROM `vw_export_datastreams`, `cfg_load_datavalues_day_sum` 
WHERE 
  `vw_export_datastreams`.`datapoints_config$0$params$query$datastream_id` = `cfg_load_datavalues_day_sum`.`DatastreamID`
;
