CREATE OR REPLACE VIEW `vw_export_datastreams_seasonal_min` AS
SELECT 
...
`datapoints_config$0$params$query$datastream_id`+10000 AS `datapoints_config$0$params$query$datastream_id`,
...
`description`,
`enabled`,
CAST(`datapoints_config$0$params$query$datastream_id`+10000 AS CHAR(50)) AS`external_refs$0$identifier`,
...
CONCAT(TRIM(`name`)," Seasonal Minimum") AS `name`,
`organization_id`,
...
`tags$1`,
`tags$2`,
"ds_Aggregate_Minimum" AS `tags$3`,
