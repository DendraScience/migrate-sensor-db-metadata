...
//ann_path = '../compost/migration3-incidents/'+org+'/wendy_done/'  

// Run through migration3 annotation directory gather list of path & files
ir_path = '../compost/migration3-incidents/'+org+'/wendy/'
ir_filepath_list = []
...
// Pull ODM ID & MongoID from each station
...
for (var l=0;l<ir_filepath_list.length;l++) {
	...
	ann.created_by = "Wendy Baxter"
	ann.created_at = ir.StartTime
	ann.actions = []
	ann.station_ids = []
	ann.datastream_ids = []
	ann.external_refs = []
	...
	ir_stations = ir.StationIDs // string2array(ir.StationIDs)
	...
	ir_datastreams =  ir.DatastreamIDs // string2array(ir.DatastreamIDs)
  ...
  if(typeof ir.flag !== 'undefined') {
	...
	// Write to file
	irpadded = 601+l
	ann_file = org+"_"+ann.created_at.substring(0,13).replace(" ","h")+".odm."+irpadded+".annotation.json"
	ann_string = JSON.stringify(ann,null,2)
	//console.log(ann_string)
	fs.writeFileSync(ann_path+ann_file,ann_string,'utf-8')
	console.log("\t exporting:",ann_path+ann_file)
}
